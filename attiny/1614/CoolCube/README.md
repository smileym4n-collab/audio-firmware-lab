...
- Startup keeps LM1971 muted until after TDA standby and mute are released.
- Startup then ramps from LM1971 mute/minimum volume to the current pot level.
...
- Pot readings from ADC 0..120 send true mute, with release delayed until ADC 150
...
- `0x3F` and above = mute
- ADC `0..120` = mute
- ADC `121..150` = stay muted if already muted
- ADC `151..1023` = mapped from `0x3E` to `0x00`
...
5. Write LM1971 mute several times.
6. Wait about 2000 ms for rails, DAC, VREF and analogue sections to settle.
7. Read the pot and calculate the target LM1971 level, but leave LM1971 muted.
8. Release standby by driving `AMP_STBY_CTRL` LOW.
9. Wait about 500 ms.
10. Release TDA mute by driving `AMP_MUTE_CTRL` LOW while LM1971 is still muted.
11. Wait about 250 ms.
12. Ramp LM1971 one code at a time to the target pot level.
...
- power-fail threshold is about 11.5 V battery input, raw ADC about 414
- recovery/hysteresis threshold is about 12.0 V battery input, raw ADC about 432

Power-fail detection uses a single fresh VBAT reading. It trips on either the
absolute low-voltage threshold or a fast raw-ADC drop, so shutdown can begin
before the amplifier rail collapses. Once detected, the fault is latched until
reset.

On power-fail:

1. `AMP_MUTE_CTRL` is driven HIGH immediately.
2. The LM1971 is commanded to mute immediately if control power remains alive.
3. After about 50 ms, `AMP_STBY_CTRL` is driven HIGH.
...
- Battery sense measures the incoming `VBAT_IN` side before the amplifier bulk
  rail collapses.
- The 11.5 V power-fail threshold is intentionally close to normal 12 V
  operation so the amplifier is muted early when input power is removed.
